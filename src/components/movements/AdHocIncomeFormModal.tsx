...
import { validateAdHocIncome } from "@/lib/finance/ad-hoc-income";
...
const emptyForm = (): AdHocIncomeFormData => ({
  name: "",
  amount: 0,
  date: defaultDate(),
  active: true,
...
export function AdHocIncomeFormModal({
  open,
  initial,
  onClose,
  onSubmit,
  ...
  initial?: AdHocIncome;
  onClose: () => void;
  onSubmit: (data: AdHocIncomeFormData) => void;
}) {
  const [form, setForm] = useState<AdHocIncomeFormData>(emptyForm());
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    const validation = validateAdHocIncome(form);
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/40 p-4 sm:items-center">
      ...
      >
        <h3 className="text-xl font-bold text-slate-900">
          {initial ? "Editar ganho" : "Registrar ganho extra"}
        </h3>
        <p className="mt-1 text-sm text-slate-500">
          Freela, venda, presente — dinheiro que entrou fora do salário fixo.
        </p>
        ...
        <div className="mt-6 flex gap-3">
          ...
          <button
            type="submit"
            className="flex-1 rounded-2xl bg-emerald-600 py-3 font-medium text-white"
          >
            Salvar
          </button>
        </div>
